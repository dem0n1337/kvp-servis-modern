<template>
  <div
    :class="
      cn(
        'relative inline-block h-10 w-full overflow-hidden rounded-lg p-0.5 z-10',
        props.class,
      )
    "
  >
...
.neon-border-one {
  ...
  width: 100%;
  height: 100%;
  filter: blur(0.5px) drop-shadow(0 0 4px v-bind(colorType1));
  z-index: -1;
  inset: 0;
}
...
.neon-border-two {
  ...
  width: 100%;
  height: 100%;
  filter: blur(0.5px) drop-shadow(0 0 4px v-bind(colorType2));
  z-index: -1;
  inset: 0;
}
